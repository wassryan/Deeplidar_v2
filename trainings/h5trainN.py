# CUDA_VISIBLE_DEVICES=2,9 python h5trainN.py --batch_size 2 --gpu_nums 2 (batch_szie must match gpu_nums)
...
import sys
sys.path.append('../')
from dataloader import h5normalLoader
...
# parser.add_argument('--loadmodel', default='/home/zhengk/DeepLiDAR/depth_completion_KITTI.tar',
#                     help='load model')
parser.add_argument('--loadmodel', default=None,
                    help='load model')
parser.add_argument('--savemodel', default='ttmy',
                    help='save model')
...
TrainImgLoader = torch.utils.data.DataLoader(
    h5normalLoader.H5Dataset('/nfs-data/zhengk_data/kitti_hdf5/train/'),
...
def train(inputl,sparse,mask,mask1,gt1):
        ...
        # set_trace()
        inputl = Variable(torch.FloatTensor(inputl)) # rgb
        gt1 = Variable(torch.FloatTensor(gt1)) # surface normal(gt depth)
        # print(sparse.dtype) # torch.float32 ->float (ps: torch.float64->double)
        sparse = Variable(torch.FloatTensor(sparse)) # lidar depth
        mask = Variable(torch.FloatTensor(mask)) # lidar depth mask
        mask1 = Variable(torch.FloatTensor(mask1)) # surface normal mask
        ...
        '''
        print("inputl: ",inputl.size()) # (12, 3, 256, 512)
        print("sparse: ",sparse.size()) # (12, 1, 256, 512)
        print("gt1: ",gt1.size()) # (12, 256, 512, 3)
        print("mask: ", mask.size()) # (12, 256, 512, 1)
        print("mask1: ",mask1.size()) # (12, 256, 512, 3)
        '''
        # set_trace()
