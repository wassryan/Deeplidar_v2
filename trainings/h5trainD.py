# CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7,8 python trainD.py --loadmodel D_model.tar --epochs 12 --batch_size 8 --gpu_nums 8
...
parser.add_argument('--savemodel', default='D_model', help='save model')
...
TrainImgLoader = torch.utils.data.DataLoader(
        h5dataLoader.H5Dataset('/nfs-data/zhengk_data/kitti_hdf5/train/'), # zk edit to H5Dataset
...
def train(inputl,gt1,sparse,mask,params):
        ...
        outC, outN, _, _, normals2 = model(inputl, sparse, mask) # zk_add
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, (imgL_crop,input_crop1,sparse2,mask2,params) in enumerate(TrainImgLoader):
            ...
            if iter_num % 100 == 0:
                num += 1
                writer.add_scalar('total_loss', iter_loss/100, num)
                iter_loss = 0
        ...
        #SAVE
        if epoch % 1 == 0:
            savefilename = args.savemodel + '_' + str(epoch) + '.tar'
            torch.save({
	            'epoch': epoch,
