...
parser.add_argument('--loadmodel', default='./Final_model.tar',
                    help='load model')
...
# writer.add_graph(model, (dummy_left, dummy_sparse, dummy_mask))
...
def test(imgL,sparse,mask):
        ...
        with torch.no_grad():
            ...
            outC, outN, maskC, maskN, con_mask = model(imgL, sparse, mask)
...
def main():
   ...
   for inx in range(len(left_test)):
       ...
       output1 = '/home/zhengk/DeepLiDAR/output/' + left_test[inx].split('/')[-1]
