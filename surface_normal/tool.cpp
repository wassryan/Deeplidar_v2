...
#include<typeinfo>
float fcxcy[3]={0,0,0};
//float fcxcy[3]={718.856, 607.1928, 161.2157};
//15 0.1 for kitti lidar, 7 0.1 for nyu
//(data=="2011_10_03"){fcxcy[0]=718.856 ;fcxcy[1]= 607.1928;fcxcy[2]= 161.2157;}
int  WINDOWSIZE =15;
float Tthrehold = 0.1;
...
MkDir(string stdirname){
	...
	while(flag){
		...
		//cout<<"stdirname:"<<stdirname<<endl;
		caps.push_back(firstname);
		if(stdirname.size()<=1)
		flag=false;
	}
	//cout<<"caps[0]:"<<caps[0]<<endl;
	int cengji=caps.size();
	//cout<<"cengji:"<<cengji<<endl;
	for (int i= 0;i<cengji;i++){
		string dir_name;
		for(int j=0;j<=i;j++){
			//cout<<"caps["<<j<<"]"<<caps[j]<<endl;
...
int main()
{
    ...
    string normal_data = "/workspace/normal_data";
    std::cout << "begin!!!!!!" << std::endl;
    vector<string> fileList = ReadDir(data + "/kitti_depth/train/");
    ...
    {
        get_pic_para(fileList[i], fcxcy);
        cout<<"fcxcy: " <<fcxcy[0]<<" "<<fcxcy[1]<<" "<<fcxcy[2]<<endl;
        cout<<"fileList[i]"<<fileList[i]<<endl;
	    vector<string> fileList02 = ReadDir(fileList[i] + "/proj_depth/");

	    for(int j=0;j<fileList02.size(); j++) // ground_truth/vvelodyne_raw
	    {
                cout<<"fileList02[i]:"<<fileList02[j]<<endl;
	            vector<string> fileList03 = ReadDir(fileList02[j] + "/");
            	//MkDir(normal_data + fileList02[j]);
                for(int k=0;k<fileList03.size();k++)//image02/image03
                {
                    cout<<"fileList03[k]"<<fileList03[k]<<endl;
            	    MkDir(normal_data + fileList03[k]);

                    vector<string> imgList = ReadDir(fileList03[k] + "/");
                    // travel images
                    for(int p=0; p<imgList.size(); p++)
                    {
                        cout<< "img: "<<imgList[p]<<endl;
                        Mat src=imread(imgList[p], CV_LOAD_IMAGE_ANYDEPTH);
                        Mat res=calplanenormal(src);
                        imwrite(normal_data + imgList[p], res);
                    }
                }
    ...
    */
    ...
    Mat src=imread("./lidar.png",CV_LOAD_IMAGE_ANYDEPTH);
    Mat res=calplanenormal(src);
    cout<< "cols: "<<res.cols <<"rows: "<< res.rows<<endl;
    cout<< "type: "<<res.type()<<endl;
    cout<< "depth: "<<res.depth()<<endl;
    cout<< "channels: "<<res.channels()<<endl;
    imwrite("./lidar_out.png",res);
